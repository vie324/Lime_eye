...
const COLUMN_MAP = {
  lumiss: [
    ...
    'visit_frequency',
    'interested_menu',
    'glue_allergy_history',
    'eye_trouble_current',
    'health_condition',
    'recent_procedures_3m',
    'allergy',
    'past_inflammation',
    'eye_appearance_pref',
    'design_pref',
  ...
  ssin: [
    ...
    'eye_appearance_pref',
    'lash_design_image',
    'homecare_product_lash',
    'sns_photo_ok_lash',
    'consent_lash',
    // --- アイブロウ+ラッシュリフト併用系 ---
    'eye_surgery_history_combo',
    'allergy_combo',
    'skin_concern_combo',
    'brow_self_care_combo',
    'last_brow_care_timing_combo',
    'brow_color_bleach_combo',
    'brow_design_image_combo',
    'desired_impression_combo',
    'brow_makeup_items_combo',
    'glue_allergy_history_combo',
    'contact_lens',
    'eye_appearance_pref_combo',
    'lash_design_image_combo',
    'homecare_product_combo',
    'sns_photo_ok_combo',
    'consent_combo',
    'first_visit_homecare',
    'age',
...
const LABELS = {
  lumiss: {
    ...
    eye_trouble_current: '目元トラブル（現在）',
    health_condition: '体調（生理/妊娠/授乳）',
    recent_procedures_3m: '3ヶ月以内の施術歴',
    allergy: 'アレルギー',
    past_inflammation: '過去の炎症経験',
    eye_appearance_pref: '目の見え方の希望',
    design_pref: 'デザイン希望',
    design_image_pref: 'デザインイメージ',
    homecare_product: 'ホームケア商品',
    sns_photo_ok: 'SNS写真使用',
    consent: '施術同意',
...
function getList(salon, limit, offset, search) {
  ...
  for (let i = data.length - 1; i >= 1; i--) {
    ...
    record.timestamp = formatDate(row[0]);
    record.name = row[1] || '';
    record.phone = row[3] || '';
    record.birthday = row[4] || '';
    record.how_found = row[6] || '';
    record.visit_frequency = row[10] || '';

    if (salon === 'ssin' && row[13]) {
      record.treatment_planned = row[13];
    }
